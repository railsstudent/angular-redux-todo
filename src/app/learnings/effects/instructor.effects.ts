...
export class InstructorEffects {
  ...
  addInstructor$: Observable<Action> = this.actions$.pipe(
    ofType<instructorActions.AddInstructorAction>(
      instructorActions.ADD_INSTRUCTOR
    ),
    ...
    mergeMap((newInstructor: InstructorModel) => {
      if (newInstructor.name !== "Bad Instructor") {
        return of(newInstructor).pipe(
          ...
          catchError(error =>
            of(new instructorActions.AddInstructorFailedAction({ error }))
          )
        );
      }
      // Fake error message
      return _throw(`Unable to add instructor ${newInstructor.name}`).pipe(
        catchError(error =>
          of(new instructorActions.AddInstructorFailedAction({ error }))
        )
      );
  ...
  deleteInstructor$: Observable<Action> = this.actions$.pipe(
    ofType<instructorActions.DeleteInstructorAction>(
      instructorActions.DELETE_INSTRUCTOR
    ),
    map(
      (action: instructorActions.DeleteInstructorAction) => action.payload.id
    ),
    mergeMap((instructorId: string) => {
      ...
      return _throw(`Unable to delete instructor id ${instructorId}`).pipe(
        catchError(error =>
          of(new instructorActions.DeleteInstructorFailedAction({ error }))
        )
      );
  ...
  updateInstructor$: Observable<Action> = this.actions$.pipe(
    ofType<instructorActions.UpdateInstructorAction>(
      instructorActions.UPDATE_INSTRUCTOR
    ),
    ...
    concatMap((updatedInstructor: InstructorModel) => {
      if (updatedInstructor.id !== "1") {
        return of(updatedInstructor).pipe(
          delay(DELAY_TIME),
          map(
            data => new instructorActions.UpdateInstructorSuccessAction(data)
          ),
          catchError(error =>
            of(new instructorActions.UpdateInstructorFailedAction({ error }))
          )
        );
      }
      return _throw(
        `Uable to update instructor ${updatedInstructor.name}`
      ).pipe(
        catchError(error =>
          of(new instructorActions.UpdateInstructorFailedAction({ error }))
        )
